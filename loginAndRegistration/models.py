...
import re, bcrypt, random, string
...
class UserManager(models.Manager):
    def UserRegisterValidator(self,data,session):
        ...
        if not errors:
            ...
            User.objects.create(first_name=data['first_name'], last_name=data['last_name'],
                                email=data['email'], password=password_hash, session=session, api_key=generate_key(40))
    ...
    def UserLoginValidator(self,data,session):
        ...
        if not errors:
            ...
            if not bcrypt.checkpw(data['password'].encode('UTF-8'), this_user.password.encode('UTF-8')[2:-1]):
